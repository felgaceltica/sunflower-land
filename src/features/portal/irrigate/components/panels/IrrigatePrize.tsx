...
const _dailyHighscore = (state: PortalMachineState) => {
  ...
  const minigame = state.context.state?.minigames.games["irrigate"];
  const history = minigame?.history ?? {};

  return history[dateKey]?.highscore ?? 0;
};
const _prize = (state: PortalMachineState) => {
  return state.context.state?.minigames.prizes["irrigate"];
};

export const IrrigatePrize: React.FC = () => {
    ...
    <OuterPanel>
      <div className="px-1">
        <span className="text-xs mb-2">
          {t("irrigate.portal.missionObjectives", {
