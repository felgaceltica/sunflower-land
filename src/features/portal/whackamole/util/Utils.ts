...
export function drawHitbox(
  ...
  if (!gameObject.input) {
    //console.warn("O gameObject não está interativo. Chame setInteractive() antes de desenhar a hitbox.");
    return;
  }
  ...
  if (hitArea instanceof Phaser.Geom.Circle) {
    const centerX =
      gameObject.x +
      (hitArea.x - gameObject.originX * gameObject.width) * gameObject.scaleX;
    const centerY =
      gameObject.y +
      (hitArea.y - gameObject.originY * gameObject.height) * gameObject.scaleY;
    const radius = hitArea.radius * gameObject.scaleX;
    ...
    const rectY =
      gameObject.y +
      (hitArea.y - gameObject.originY * gameObject.height) * gameObject.scaleY;
    const rectWidth = hitArea.width * gameObject.scaleX;
    const rectHeight = hitArea.height * gameObject.scaleY;
