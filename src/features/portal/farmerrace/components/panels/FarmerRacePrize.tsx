...
const _dailyHighscore = (state: PortalMachineState) => {
  ...
  const minigame = state.context.state?.minigames.games["farmer-race"];
  const history = minigame?.history ?? {};

  return history[dateKey]?.highscore ?? 0;
};
const _prize = (state: PortalMachineState) => {
  return state.context.state?.minigames.prizes["farmer-race"];
};

export const FarmerRacePrize: React.FC = () => {
      ...
      <OuterPanel>
        <div className="px-1">
          <Label type="danger" icon={SUNNYSIDE.icons.sad}>
            {t("farmer-race.noPrizesAvailable")}
          </Label>
        </div>
      </OuterPanel>
    );
    ...
    <OuterPanel>
      <div className="px-1">
        <span className="text-xs mb-2">
          {t("farmer-race.portal.missionObjectives", {
